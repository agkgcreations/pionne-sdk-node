...
- **Actionable error message on permanent ingest rejection.** `send()`
  ...
  validation), and emits a `console.warn` (once per session, in
  prod too). Includes the `app_id` actually sent and the masked
  `expected_format` returned by the server. Fixes the silent rejection
  footgun where a misconfigured `PIONNE_TOKEN` or stale bundle pinning
  would drop events without any visible signal in the host process'
  stderr.
